...
export function reactViews(reactViewOptions: ReactViewsOptions) {
  return async function renderFile(
    ...
    const { settings, _locals, cache, contexts, ...userVars } =
      options as ExpressRenderOptions;
    const folderPathOfFile = getDirPathFromFullPath(filename);
    if (!WEBPACK_CONFIG) {
      const configPath = path.join(folderPathOfFile, WEBPACK_MERGED_CONFIG_TMP);
      try {
        if (fs.existsSync(configPath)) {
          WEBPACK_CONFIG = JSON.parse(fs.readFileSync(configPath, "utf-8"));
        }
      } catch (e) {}
    }
    try {
      const rawComponentStr = `
        <script id="${INJECT_DATA_SCRIPT_ID}" data-obj="${encodeURIComponent(
        JSON.stringify(userVars)
      )}"></script>
        <script>${fs.readFileSync(filename, "utf-8")}</script>
      `;
      const transform =
        reactViewOptions.transform ||
        ((componentFragment) => {
          let html = "";
          const injectCssList =
            getCssStringFromBaseFolderPath(folderPathOfFile);
          const injectCssStr = injectCssList.reduce(
            (accu: string, cssStrWithTag: string) => {
              accu += cssStrWithTag;
              return accu;
            },
            ""
          ...
          const injectScriptTag = generateInjectScriptTag({
            injectScript: externalConfig?.injectScript,
          });
          const injectStyleTag = generateInjectStyleTag({
            injectStyle: externalConfig?.injectStyle,
          });
          const commonVendorScriptList = getCommonVendorFromBaseFolderPath(
            folderPathOfFile,
            {
              publicPath: WEBPACK_CONFIG?.output?.publicPath,
            }
          );
          let commonVendorScriptString = "";
          if (commonVendorScriptList?.length) {
            commonVendorScriptList?.forEach((scriptTag) => {
              commonVendorScriptString += scriptTag;
            });
          }
