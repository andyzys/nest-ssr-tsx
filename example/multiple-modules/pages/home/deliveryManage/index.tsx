...
export default function DeliveryManageHome(props) {
  // console.log('拿到的参数是', props)
  const { file } = props;
  const [defaultChannelDeliveryList, setDefaultChannelDeliveryList] = useState([]);
  const [defaultAppNameMap, setDefaultAppNameMap] = useState({});

  useEffect(() => {
    getKConfBuriedConfig().then((list: any) => {
      setDefaultChannelDeliveryList(list[0])
      setDefaultAppNameMap(list?.[1]?.appNameMap)
    }).catch(err => {
      message.info(err)
    });
  }, [])
