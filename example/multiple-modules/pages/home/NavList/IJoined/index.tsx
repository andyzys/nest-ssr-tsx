...
import { FANGZHOU_PASS_NAV_I_JOINED, SELECTED_BY_DEFAULT, templateNamespaces } from '../../../common/const'

// @ts-ignore
import css from '../nav.less'

export class IJoinedCtx {
  open: boolean
  loading: boolean
...
export default function({
  ...
  const ctx: IJoinedCtx = useObservable(IJoinedCtx, next => {
    function getLocalInfo () {
      return getLocalStorage(FANGZHOU_PASS_NAV_I_JOINED) || {}
    }

    const info = Object.assign(
      {open: true, dataSource: []},
      getLocalInfo()
    )
        ...
        if (homePageCtx.currentClick) {
          if (homePageCtx.isGroup(homePageCtx.currentClick)) {
            if (currentClickId === id) {
              homePageCtx.currentClick.navType = 'iJoined'
            }
          } else {
            if (groupId === id) {
              homePageCtx.currentClick.navType = 'iJoined'
    ...
    homePageCtx.createGroup({user: homePageCtx.user}).then(() => {
      ...
      homePageCtx.getUserGroups({params: {
        ...
        ctx.dataSource.forEach((data) => {
          ...
          if (info?.open) {
            data.open = true
            data.dataSource = info.dataSource
          }
        })
      })
    })
  ...
  const clickSwitcher = useCallback(async (item?: any) => {
    ...
    setLocalStorage(FANGZHOU_PASS_NAV_I_JOINED, {
      open: ctx.open,
  ...
  const clickWrapper = useCallback(async (item?: any) => {
    if (!item) {
      ...
      setLocalStorage(SELECTED_BY_DEFAULT, {...item, navType: 'iJoined'})
      
      const { id, extName, groupId } = item
      const { id: currentId, extName: currentExtName } = homePageCtx.currentClick || {}

      homePageCtx.currentClick = {...item, navType: 'iJoined'}
      if (extName !== currentExtName) {
        if (!item.open) {
          item.loading = true
        }

        await ctx.fetch({linkageWithHome: true, item: {...item, navType: 'iJoined'}})
